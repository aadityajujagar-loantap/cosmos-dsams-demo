...
export function Phase2LeadManagementScreen() {
  ...
  const loadAllData = async () => {
    try {
      setLoading(true);
      const [leadsRes, makerRes, reportsRes, prodRes, titleRes, genderRes, empRes, occRes, devRes] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Phase 2 Lead Creation, Branch Maker Queue, Checker Workstation, Multi-Facility Loan Account Mapping & Reports."
        eyebrow="Phase 2 Lead Management"
        title="Lead Lifecycle & Workstation"
      />
      ...
      {/* Create Lead Modal */}
      <Modal open={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Create New Lead" width="max-w-2xl">
        <form onSubmit={handleCreateLead} className="space-y-4 text-xs">
          
          {/* STEP 1: Product Selection */}
          <div className="bg-slate-50 p-3 rounded-lg border space-y-2">
            <p className="font-bold text-slate-700 uppercase tracking-wider text-[11px]">1. Select Product & Loan Type</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-slate-600 font-semibold mb-1">Select Loan Type *</label>
                <select
                  required
